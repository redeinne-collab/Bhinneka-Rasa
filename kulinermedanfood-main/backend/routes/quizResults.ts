...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.query(sql`
      ...
      RETURNING id
    `);
    const newRow = await db.query(sql`SELECT * FROM quiz_results WHERE id = ${result[0].id}`);
...
router.get('/user/:userId', async (req: Request, res: Response) => {
  try {
    const rows = await db.query(sql`SELECT * FROM quiz_results WHERE user_id = ${req.params.userId} ORDER BY created_at DESC`);
