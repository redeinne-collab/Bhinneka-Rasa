...
import { LogIn, User, Star, Trophy, Settings, LogOut, ChevronRight, Loader2, Award, BookOpen } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const mainQuizResult = quizResults.find(r => r.quiz_type === 'main')
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <StatCard
            ...
            value={mainQuizResult ? mainQuizResult.total_score : 0}
            ...
            {loading ? (
              ...
              <div>
                ...
                <div className="space-y-3 mb-5">
                  ...
                  {[
                    { label: 'Kari Bihun', score: personalityResult.score_kb },
                    { label: 'Soto Medan', score: personalityResult.score_sm },
                    { label: 'Bolu Meranti', score: personalityResult.score_bm },
                    { label: 'Bika Ambon', score: personalityResult.score_ba },
                    { label: 'Cicongfan', score: personalityResult.score_cf }
                  ].map((item, idx) => (
            ...
            {loading ? (
              ...
              <div>
                <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl p-5 text-white mb-5 shadow-lg text-center">
                  ...
                  <p className="text-white/90 text-sm">
                    Diselesaikan pada {formatDate(mainQuizResult.created_at)}
                  </p>
                ...
                <div className="space-y-3 mb-5">
                  <h4 className="font-bold text-gray-800 text-sm mb-2">Detail Skor Per Kuliner:</h4>
                  {[
                    { label: 'Soto Medan', score: mainQuizResult.score_kb },
                    { label: 'Ci Cong Fan', score: mainQuizResult.score_sm },
                    { label: 'Bika Ambon', score: mainQuizResult.score_bm },
                    { label: 'Kari Bihun', score: mainQuizResult.score_ba },
                    { label: 'Bolu Meranti', score: mainQuizResult.score_cf }
                  ].map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <span className="text-xs font-semibold text-gray-600 w-24">{item.label}</span>
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl mb-6">
          ...
          <div className="space-y-2">
            <MenuItem
              ...
              onClick={() => navigate('/settings')} // <-- TAMBAHKAN INI
            />
            <MenuItem
              icon={<User className="w-5 h-5 text-gray-500" />}
              label="Tentang Aplikasi"
              onClick={() => navigate('/about')} // <-- TAMBAHKAN INI
            />
          </div>
        </div>
